...
def features():
    ...
    for movie_div in movie_divs:
        ...
        title_elem = movie_div.find('h5', class_='truncate')
        movie_title = title_elem.text.strip() if title_elem else 'N/A'

        image_elem = movie_div.find('img', loading='lazy')
        img_src = image_elem['src'] if image_elem and 'src' in image_elem.attrs else 'N/A'

        movie_list.append({'title': movie_title, 'image_url': img_src})

    return render_template('rules.html', movies=movie_list)
